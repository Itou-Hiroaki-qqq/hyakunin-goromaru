...
4. **ログイン** - `/login`
5. **トップ** - `/`（タイトル・学習スタート・復習ボタン・実践問題ボタン・コンピューター対戦ボタン、フッターのみ）
   - **復習**ボタンは、学習リストや間違えやすい問題のテストで1問でも間違えているときのみ有効（リンク）。それ以外は無効表示。
   - **実践問題**・**コンピューター対戦**ボタンは、「100首ぜんぶテスト」を全問一発正解でクリアすると有効になる。それ以外は無効表示（解放条件テキストを併記）。
...
13. **コンピューター対戦** - `/battle`
    ...
    - **ステージ選択**: 初級（語呂終了後4秒）・中級（2秒）・上級（0.5秒）の3段階。
    - **問題数選択**: 20問・40問・60問・80問・100問から選択。選択後「対戦スタート」ボタンが表示。
    - **対戦形式**: 100首からランダムで問題数分を選出。上の句音声が流れ、下の句4択（PC横並び / SP 2列）。
    - コンピューターは各句の語呂が終わる秒数（`src/data/goro-timings.ts`）＋ステージ遅延で正解札を取る。取得アニメーションあり（`public/hand.png`）。
    - **お手付き**: 不正解の札を選ぶとそれ以上選択不可。コンピューターが取るのを待つ。
    - いずれかが正解（または取得アニメーション終了）後「次へ」ボタンが出現。2問目以降は1秒後に音声が流れる。
    - **結果画面**: コンピュータースコア → あなたのスコア → 勝者を逐次表示（タップでスキップ可）。同点時は「同点！」。「もう一度挑戦する」「ステージ選択へ戻る」「トップへ戻る」ボタンあり。
12. **実践問題** - `/jissen`
    - 「100首ぜんぶテスト」を全問一発正解でクリアするとトップから解放される。
...
- **コンピューターの手イラスト**: `public/hand.png`（指が下方向の手の画像）。差し替える場合は同パスに上書き保存してください。
