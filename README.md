...
6. **学習リスト** - `/learn`（1～100首を4首単位で25ブロック）
   ...
   - 展開時は縦並びで「学習」「4首でテスト」を表示。
   - **前回も入れて8首でテスト**: 5～8首・13～16首・21～24首…など、偶数番目のブロック（2ブロック目・4ブロック目…）に表示。前4首＋当該4首の計8首でテスト。
   - **ここまでのまとめテスト**: 13～16首・21～24首・29～32首…など、16首学習以降の偶数ブロックに表示。1首～当該ブロック末までのまとめテスト。
...
8. **4首／8首／まとめテスト** - `/learn/[range]/test`
   - `[range]` は `1-4`, `5-8`, …, `97-100`（4首テスト）、または `1-8`, `9-16`, …（8首テスト）、`1-16`, `1-24`, …（ここまでのまとめテスト）。
   - 上の句＋音声＋語呂赤。選択肢は正解＋当該範囲内の他首から3つ。縦書き札・畳風背景・〇×。結果画面で正解数表示。
